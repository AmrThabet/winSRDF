...
*/
static inline DWORD roll_hash(unsigned char c)
{
...
static inline DWORD sum_hash(unsigned char c, DWORD h)
{
  h *= HASH_PRIME;
  h ^= c;
  return h;
...
static int ss_update(ss_context *ctx, FILE *handle)
{
  ...
  _snprintf_s(ctx->ret, 12,  _TRUNCATE,"%u:", ctx->block_size);
...
static int has_common_substring(const char *s1, const char *s2)
{
  ...
  for (i=0;s2[i];i++) {
    DWORD h = roll_hash((unsigned char)s2[i]);
...
/*
  ...
  messages.
*/
static unsigned score_strings(const char *s1, const char *s2, DWORD block_size)
{
  DWORD score;
  DWORD len1, len2;
